...
{
  public:
    virtual void operator() (byte index, ELEMENT* pElement) = 0;
};
...
{
  void operator() (byte index, X* pElement) 
  {
    // https://www.tutorialspoint.com/c_standard_library/c_function_sprintf.htm
    char cTemp[40];
    sprintf(cTemp, "Index: %i - Value: %s", (int)index, pElement->getString());
...
{
  void operator() (byte index, char* pElement) 
  {
    char cTemp[40];
    sprintf(cTemp, "Index: %i - Value: %c", (int)index, *pElement);
    Serial.println(cTemp);
  }
} forEachGetChar;
