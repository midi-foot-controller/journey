...
class Task
{
  ...
  public:
    Task(DATATYPE interval_ms, bool running = true);
    
    virtual void updateTime(DATATYPE currentTime_ms);
    virtual void run(DATATYPE currentTime_ms) = 0;
    virtual byte getType() = 0;   // temp solution to support RTTI
...
template<class DATATYPE> 
Task<DATATYPE>::Task(DATATYPE interval_ms, bool running) : _running(running)
{  
...
class TaskCounter : public Task<DATATYPE>
{
  ...
  public:
    TaskCounter(char id, DATATYPE interval_ms, bool running = true) : Task<DATATYPE>(interval_ms, running), _id(id) {}
    
    void run(DATATYPE currentTime_ms) { _counter++; }
    
    unsigned long getCounter() { return _counter; }
    byte getId() { return _id; }
    byte getType() { return 1; }  // temp solution to support RTTI
};
...
class TaskManager : public Task<DATATYPE>
{
  ...
  public:
    ...
    byte getType() { return 2; }  // temp solution to support RTTI
